    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        if no_nexts:
            ...
            losses, generated = model(Variable(data['label']), Variable(data['next_label']), Variable(data['image']), \
                    Variable(data['next_image']), Variable(cond_zeros), lhsk_real, rhsk_real, hand_state_real, hsk_frame, infer=True)
            ...
            loss_D = (loss_dict['D_fake'] + loss_dict['D_real']) * 0.5 + (loss_dict['D_lhand_real'] + loss_dict['D_lhand_fake']) * 0.5 + (loss_dict['D_rhand_real'] + loss_dict['D_rhand_fake']) * 0.5 + (loss_dict['D_hand_real'] + loss_dict['D_hand_fake']) * 0.5
